...
process_file_change() {
    ...
    if ! ./scripts/validation/validate-form-fields.py "$file" --quiet 2>/dev/null; then
        echo -e "${RED}  ✗ Form field errors detected!${NC}"
        ./scripts/validation/validate-form-fields.py "$file" 2>&1
    ...
    if [[ "$file" == *source-documents/Ordinances/* ]]; then
        ...
        ./scripts/preprocessing/sync-ordinances.py >/dev/null 2>&1
    elif [[ "$file" == *source-documents/Resolutions/* ]]; then
        echo "  Syncing resolutions..."
        ./scripts/preprocessing/sync-resolutions.py >/dev/null 2>&1
    elif [[ "$file" == *source-documents/Interpretations/* ]]; then
        echo "  Syncing interpretations..."
        ./scripts/preprocessing/sync-interpretations.py >/dev/null 2>&1
    elif [[ "$file" == *source-documents/Other/* ]]; then
        echo "  Syncing other documents..."
        ./scripts/preprocessing/sync-other.py >/dev/null 2>&1
    ...
    if [ -n "$dest_file" ] && [ -f "$dest_file" ]; then
        ...
        ./scripts/preprocessing/standardize-list-format.py >/dev/null 2>&1 || true

        echo "  Fixing mixed list formats..."
        ./scripts/preprocessing/fix-mixed-list-format.py "$dest_file" >/dev/null 2>&1 || true

        echo "  Processing footnotes..."
        ./scripts/preprocessing/footnote-preprocessor.py "$dest_file" >/dev/null 2>&1 || true
        
        echo "  Converting links..."
        ./scripts/preprocessing/auto-link-converter.py "$dest_file" >/dev/null 2>&1 || true
    fi
    
    # Always update cross-references (they scan all files)
    echo "  Updating cross-references..."
    ./scripts/mdbook/add-cross-references.py >/dev/null 2>&1
    
    # Regenerate SUMMARY.md and relationships
    echo "  Updating indexes..."
    ./scripts/mdbook/generate-summary-with-airtable.py >/dev/null 2>&1
    ./scripts/mdbook/generate-relationships.py >/dev/null 2>&1
    ...
    if [[ "$file" == *.css ]] || [[ "$file" == theme/* ]]; then
        ...
        ./scripts/build/compile-css.py >/dev/null 2>&1
    fi
    ...
    ./scripts/postprocessing/enhanced-custom-processor.py >/dev/null 2>&1
    ...
    if [ -f "scripts/tests/test-list-formatting.py" ]; then
        ...
        if ! ./scripts/tests/test-list-formatting.py book/ordinances/1989-Ord-54-89C-Land-Development.html >/dev/null 2>&1; then
    ...
    if ! ./scripts/validation/check-styles-health.py --verbose 2>&1; then
...
if wait_for_server_start 3000 10; then
    ...
    ./scripts/build/compile-css.py >/dev/null 2>&1 || true
    # Run enhanced-custom-processor first for tables, WHEREAS, etc (NO list processing)
    ./scripts/postprocessing/enhanced-custom-processor.py >/dev/null 2>&1 || true
    # Use unified list processor v2 for ALL list processing
    ./scripts/postprocessing/unified-list-processor.py >/dev/null 2>&1 || true
    echo "✅ Styles applied"
    echo ""
    echo "=========================================="
